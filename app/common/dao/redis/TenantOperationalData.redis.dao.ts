...
export class TenantOperationalDataRedisDao extends IBaseRedisDao {

    async exits(uniqueDateTime: string): Promise<boolean> {
        const conn = await redisManager.getConnection(RedisDB.RuntimeData);
        try {
            return !!await conn.exists(`${DB2.tenantOperationalData}:${uniqueDateTime}`);
        } catch (e) {
            return false;
        }
    }
    ...
    async findOne(uniqueDateTime: string): Promise<Array<any>> {
        const conn = await redisManager.getConnection(RedisDB.RuntimeData);
        try {
            const game = await conn.get(`${DB2.tenantOperationalData}:${uniqueDateTime}`);
    ...
    async insertOne(uniqueDateTime: string, parameter): Promise<boolean> {
        const conn = await redisManager.getConnection(RedisDB.RuntimeData);
        try {
            const game = await conn.setex(`${DB2.tenantOperationalData}:${uniqueDateTime}`, 30, JSON.stringify(parameter));
...
export default new TenantOperationalDataRedisDao();