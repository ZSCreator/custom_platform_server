 ...
 * 判断该下注数和下注倍数是否合理
 */
export function isHaveBet(betNum, betOdd) {
 ...
 * 根据 铲子数判断关卡
 */
export function pass({ shovelNum }) {
 ...
 * 选择游戏轮盘
 */
export function selectRoulette(cur) {
	if (cur == '1') {
 ...
 * 按照游戏配置生成窗口
 */
export function generatorWindow(pass, isFreespin = false) {
	let winIds = [], freespinNum = 0, bonusNum = 0, shovelNum = 0, only;
	//获取元素生产概率{ key: 'A', value: 10000 },{ key: 'B', value: 8000 },
 ...
 * 根据生成的窗口判断消除情况
 */
export function windowClear(winIds, pass) {
	let censu = {};
	...
	const clearElement:any = util.filter(e => e.length > 0)(awardResult);
 ...
 * 根据消除将原窗口降落并补全
 */
export function completeWindow(winIds, clearElement, isFreespin, only?) {
 ...
 * 根据窗口的消除情况计算中奖情况
 */
export function windowAward({ clearElement, jackpotMoney = 0, jackpotOdd = 0, pass, totalBet = 0 }) {
 ...
 * 统计此局游戏最终结果
 */
export function finalResult({ winIds, jackpotMoney, pass, totalBet, only }) {
 ...
 * 生成小游戏的初始页面
 */
export function littlerGameWindow(pass) {
	const winIds = [];
 ...
 * 得到小游戏的结果
 */
export function littlerGameResult(pass) {
 ...
 * 玩家点击进行翻转
 */
export function click(position, userLittleGame) {
	const [x, y] = position;
...
export function noWinWindow(pass, bonusNum, freeNum?) {
	let ele, num;
	if (bonusNum > 0) {
		ele = 'S';
		num = bonusNum;
...
export function sortResult(ans) {
	ans.sort((a, b) => {
