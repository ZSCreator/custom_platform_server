 ...
 * 判断下注是否合理
 */
export const isHaveBet = (bet) => config.bets.includes(bet);
 ...
 * 按照游戏配置生成窗口
 */
export const generatorWindow = ({ newer, iR1, iR2, wR, aR, AgR, canjackpotB, canjackpotS, down, curRoulette, maxAward, linePosition, filterEle }) => {
 ...
 * 根据生成的窗口判断赢钱情况
 */
export const windowAward = ({ winIds, bet, lineNum, roomJackpot, bonusNum }) => {
	...
	curWinningLines.forEach((line, item) => {
		...
		//判断这条连线的中奖情况
		const lineResult: any = winningJudgment(lineIds);
		if (lineResult != null) {
