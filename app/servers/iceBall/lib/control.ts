...
import {Lottery, IceBallLotteryResult} from "./util/lotteryUtil";
...
export default class ControlImpl extends SlotGameControl<LimitConfigManager> {
    ...
    public static getControlInstance(): ControlImpl {
        if (this.instance === null) {
            this.instance = new ControlImpl({room: { nid: GameNidEnum.IceBall, sceneId: 0 }} );
        }
    ...
    async runControl(player: Player, lotteryUtil: Lottery): Promise<IceBallLotteryResult> {
        let result: IceBallLotteryResult, controlType: ControlKinds;
