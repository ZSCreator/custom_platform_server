...
import {Lottery, PirateResult} from "./util/lotteryUtil";
...
export default class ControlImpl extends SlotGameControl<LimitConfigManager> {
    ...
    public static getControlInstance(): ControlImpl {
        if (this.instance === null) {
            this.instance = new ControlImpl({room: { nid: GameNidEnum.pharaoh, sceneId: 0 }} );
        }
    ...
    async runControl(player: Player, lotteryUtil: Lottery): Promise<PirateResult> {
        let result: PirateResult, controlType: ControlKinds;
