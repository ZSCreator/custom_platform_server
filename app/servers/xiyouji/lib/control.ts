...
import {Lottery, XYJLotteryResult} from "./util/lotteryUtil";
...
export default class ControlImpl extends SlotGameControl<LimitConfigManager> {
    ...
    async runControl(player: Player, lotteryUtil: Lottery): Promise<XYJLotteryResult> {
        let result: XYJLotteryResult, controlType: ControlKinds;
        ...
        const { isOverrun }: { isOverrun: boolean } =
            this.isEarningsTransfinite({ player, profit: result.allTotalWin });
