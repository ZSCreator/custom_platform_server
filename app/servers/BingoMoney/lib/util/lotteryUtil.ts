...
export class Lottery {
    ...
    private modifyInitialWindow() {
        ...
        selectLines.forEach((line) => {
            // 这条线上的元素
            const elementLine: elementType[] = line.Line.map((l, i) => this.window[i][l - 1]);
            ...
            // 计算元素线中奖结果
            const lineResult: LineResult = this.calculateLineResult(elementLine, line.direction);

            // 如果有中奖元素 则记录下元素的坐标
            if (lineResult.elementType) {
                const coordinates: string[] = line.Line.slice(0, lineResult.linkNum).map((l, i) => `${i}-${l - 1}`);
    ...
    private calculateEarnings(window: elementType[][]): { winLines: WinLine[], totalWin: number } {
        ...
        selectLines.forEach((line, index) => {
            // 这条线上的元素
            const elementLine: elementType[] = line.Line.map((l, i) => window[i][l - 1]);
            // 先克隆一个当前元素的副本
     ...
     */
    private calculateLineResult(elementLine: elementType[], direction: number): LineResult {
        // 取第一个元素
