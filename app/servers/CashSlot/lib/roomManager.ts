 ...
 * 幸运777房间管理
 */
export class CashSlotRoomManager extends RoomManager<Room>{
     ...
     */
    async init() {
        const sceneList = getConfiguration('scenes/CashSlot').datas;
    ...
    createRoom(sceneId: number | string, roomId: string): Room {
        ...
        const sceneInfo = getConfiguration('scenes/CashSlot').datas.find(scene => scene.id === sceneId);
...
export default new CashSlotRoomManager(GameNidEnum.CashSlot, InteriorGameType.Slots, 'CashSlot');