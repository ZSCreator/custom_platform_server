...
export default class Player extends SlotMachinePlayer {
    ...
    bet(baseBet: number, lineNumber: number) {
        ...
        this.jackpot.mini += fixNoRound(baseBet * 0.002);
        this.jackpot.minor += fixNoRound(baseBet * 0.003);
        this.jackpot.Grand += fixNoRound(baseBet * 0.005);
    }
     ...
     * @param result
     */
    buildLiveRecord(result: string, freeSpin: boolean, odds: number, window: elementType[][]) {
        return {
            uid: this.uid,
