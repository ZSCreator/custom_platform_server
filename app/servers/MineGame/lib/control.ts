...
export default class ControlImpl extends SlotGameControl<LimitConfigManager> {
    ...
    public static getControlInstance(): ControlImpl {
        if (this.instance === null) {
            this.instance = new ControlImpl({ room: { nid: GameNidEnum.MineGame, sceneId: 0 } });
        }
    ...
    async runControl(player: Player, lotteryUtil: Lottery): Promise<{ result: boolean, limit: number }> {
        let result: boolean;
        let controlType: ControlKinds;
        ...
        // 计算这次具体收益是否超限
        const { isOverrun, limit }: { isOverrun: boolean, limit: number } =
            this.isEarningsTransfinite({ player, profit: 0 });
     ...
     */
    private runPersonalControl(lotteryUtil: Lottery, personalControlPlayers: PersonalControlPlayer[]) {
