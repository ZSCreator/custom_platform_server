import {Application, BackendSession, Logger, RemoterClass} from 'pinus';
...
import {PlayerGameState} from "../lib/constant";
...
export class MainRemote {
    ...
    public async entry({ player, roomId, sceneId }) {
        this.logger.debug(`DeadBook|进入房间:${roomId}|场:${sceneId}|玩家:${player.uid}`);
        ...
        try {
            ...
            this.logger.error(`DeadBook|进入房间:${roomId}|场:${sceneId}|玩家:${player.uid}|出错:${error}`);
    ...
    async exit({  sceneId, roomId, player }) {
        ...
        this.logger.debug(`DeadBook|离开房间:${roomId}|场:${sceneId}|玩家:${player.uid}`);
        ...
        try {
            ...
            if (!room) {
                apiResult.code = 200;
                this.logger.error(`DeadBook|离开房间未找到房间: ${roomId}|场:${sceneId}|`);
                return apiResult;
            }
            ...
            // 结算
            if (currPlayer.gameState === PlayerGameState.BO) {
            ...
            this.logger.error(`DeadBook|离开房间:${roomId}|场:${sceneId}|玩家:${player.uid}|出错:${error}`);
            apiResult.code = 200;
    ...
    public async leave({ uid, language, sceneId, roomId }) {
        ...
        this.logger.debug(`DeadBook|玩家掉线|房间:${roomId}|场:${sceneId}|玩家:${uid}`);
        ...
        try {
            ...
            this.logger.error(`DeadBook|玩家掉线|房间:${roomId}|场:${sceneId}|玩家:${uid}`, error.stack);
            // 错误把房间返回200
            apiResult.code = 200;
    ...
    public async rpcLowerPlayer({ uid, sceneId, roomId }) {
        this.logger.debug(`DeadBook|玩家游戏中下分|房间:${roomId}|场:${sceneId}|玩家:${uid}`);
        ...
        try {
            ...
            this.logger.error('DeadBook|async  rpcLowerPlayer==>', error);
            return { code: 200 };
        }
    }
