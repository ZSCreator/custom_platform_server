...
import DeadBookGameManager from "./lib/deadBookGameManager";
...
class Lifecycle implements ILifeCycle {
    async beforeStartup(app: Application, cb: () => void) {
         ...
         * 初始化游戏以及房间和场
         */
        await new DeadBookGameManager(GameNidEnum.DeadBook).init();
        // }
        cb();
    }
    ...
    async afterStartup(app: Application, cb: () => void): Promise<any> {
        ...
        // 初始化调控
        await GameControlService.getInstance().init({nid: GameNidEnum.DeadBook});
