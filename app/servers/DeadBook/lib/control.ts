...
import {BoLotteryUtil, BoResult, Lottery, SlotResult} from "./util/lotteryUtil";
...
export default class Control extends SlotGameControl<LimitConfigManager> {
    ...
    async runControl(player: Player, lotteryUtil: Lottery): Promise<SlotResult> {
        ...
        const { isOverrun }: { isOverrun: boolean } =
            this.isEarningsTransfinite({ player, profit: totalWin });
     ...
     * @param lotteryUtil
     */
    async boControl(player: Player, lotteryUtil: BoLotteryUtil): Promise<BoResult> {
        let result: BoResult, controlType: ControlKinds;
     ...
     * 运行博一博个控
     * @param lotteryUtil
     * @param personalControlPlayers
     */
    private runBoPersonalControl(lotteryUtil: BoLotteryUtil, personalControlPlayers:  PersonalControlPlayer[]): any | null {
        // 判断玩家是否满足个控概率
        const [controlPlayer] = this.filterNeedControlPlayer(personalControlPlayers);
     ...
     */
    private runSceneControl(lotteryUtil: Lottery | BoLotteryUtil, sceneControlState: ControlState): any {
