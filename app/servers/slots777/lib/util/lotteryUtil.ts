...
export class Lottery {
    ...
    private calculateEarnings(window: elementType[][]): {winLines: WinLine[], totalWin: number} {
        ...
        selectLines.forEach((line, index) => {
            ...
            if (lineResult.elementType) {
                ...
                // 如果是跟7 相关的会扣除奖池金额
                if ((lineResult.elementType === config.anySeven
                    || config.sevenElementGroup.includes(lineResult.elementType))
    ...
    private calculateLineResult(elementLine: elementType[]): LineResult {
        ...
        const firstElement = elementLine[0],
            // 特殊元素
            belongSpecial: any = config.sevenElementGroup.includes(firstElement) ? config.anySeven
                : config.barElementGroup.includes(firstElement) ? config.anyBar : null;
        ...
        // 如果第一个元素为特殊元素 判断第二个元素是否是同类 不是则没有继续判断的价值
        if (!!belongSpecial) {
            if (belongSpecial === config.anySeven ? !(config.sevenElementGroup.includes(elementLine[1]))
                : !(config.barElementGroup.includes(elementLine[1]))) {
                return result;
            }
        } else {
        ...
        switch (true) {
            ...
            case elementLine.every(element => element === firstElement): {
                ...
                // 如果单元素为任意七 触发奖池大奖
                if (belongSpecial === config.anySeven) {
                    result.prizeType = firstElement === config.oneSeven ? 'mega'
                        : firstElement === config.twoSeven ? 'monster' : 'colossal';
                }
            ...
            case (!!belongSpecial && elementLine.every(element => {
                return belongSpecial === config.anySeven ? config.sevenElementGroup.includes(element)
                    : config.barElementGroup.includes(element);
            })): {
    ...
    private freeSpinLottery() {
        ...
        // 摇奖几次
        let len = config.freeSpinMapping[this.scatterCount];
