...
export class MainRemote {
    ...
    */
     ...
     * 进入
     * @param param0 
     * @route 7up7down.mainRemote.entry
     */
    public async entry({ nid, roomId, sceneId, player }) {
        try {
     ...
     * 离开
     * @param param0 
     * @route 7up7down.mainRemote.exit
     */
    ...
    /**
     * 掉线
     * @route 7up7down.mainRemote.leave
     */
    public async leave({ uid, language, nid, sceneId, roomId }) {
        try {
            ...
            if (!playerInfo) {
                this.roomManager.removePlayer({ uid } as PlayerInfo);
