 ...
 * @param len
 */
export function getKillPlayerCards(initCards: number[][], len: number) {
    ...
    }) : controlCards[random(0, controlCards.length - 1)];
 ...
 * @param cards
 */
export function isNiuCards(cards: number[]): boolean {
    if (cards.length !== 4) {
        return false;
    }
