...
export class qznnRoomManger extends TenantRoomManager<qznnRoom>{
    ...
    createRoom(sceneId: number | string, roomId: string) {
        const room = {
            roomId,
            sceneId,
            nid: GameNidEnum.qznn,
            serverId: pinus.app.getServerId(),
        }
        const sceneInfo: IsceneMgr = getConfiguration('scenes/qznn').datas.find(scene => scene.id === sceneId);
...
export default new qznnRoomManger(GameNidEnum.qznn, InteriorGameType.Battle, 'qznn');