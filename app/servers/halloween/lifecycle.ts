...
class Lifecycle implements ILifeCycle {
    async beforeStartup(app: Application, cb: () => void) {
        ...
        //     doc && Utils.objAssignment(halloweenMemory, doc);
        // } catch (error) {
        //     // 如果初始化失败阻塞服务器启动
         ...
         * 初始化游戏以及房间和场
         */
        await new SlotsGameManager(GameNidEnum.Halloween).init();
        // }
        cb();
    }
    ...
    async afterStartup(app: Application, cb: () => void): Promise<any> {
        ...
        // 初始化调控
        await GameControlService.getInstance().init({nid: GameNidEnum.Halloween});
