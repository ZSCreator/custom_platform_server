...
class Lifecycle implements ILifeCycle {
    async beforeStartup(app: Application, cb: () => void) {
        ...
        // 初始化游戏
        await ColorPlateGameManager.init(GameNidEnum.colorPlate);
        cb();
    };
    ...
    async afterStartup(app: Application, cb: () => void) {
        ...
        // 初始化调控
        await GameControlService.getInstance().init({ nid: GameNidEnum.colorPlate, bankerGame: true });

        // 进机器人
        robotServerController.start_robot_server(GameNidEnum.colorPlate);
        cb();
    };
