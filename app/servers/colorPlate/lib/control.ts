 ...
 * 色碟调控实现
 */
export default class ColorPlateControl extends BaseGameControl {
    room: Room;
    ...
    private getKillAreas(players: PersonalControlPlayer[]): BetAreasName[] {
        ...
        // 如果超限区域同时包含单或者双 则 选取其中调控玩家押注大的杀 相同则随机杀一个
        if (overrunBetAreas.has(BetAreasName.DOUBLE) && overrunBetAreas.has(BetAreasName.SINGLE)) {
            let singleTotalBet = 0, doubleTotalBet = 0;

            for (let controlPlayer of players) {
                const bets = this.room.getPlayer(controlPlayer.uid).getBetsDetail();
                singleTotalBet += bets[BetAreasName.SINGLE];
                doubleTotalBet += bets[BetAreasName.DOUBLE];
            }

            if (singleTotalBet === doubleTotalBet) {
                Math.random() > 0.5 ? overrunBetAreas.delete(BetAreasName.DOUBLE) : overrunBetAreas.delete(BetAreasName.SINGLE);
            } else {
                singleTotalBet > doubleTotalBet ? overrunBetAreas.delete(BetAreasName.DOUBLE) : overrunBetAreas.delete(BetAreasName.SINGLE);
            }
        }
