...
export class ErRoomManger extends TenantRoomManager<ErbaRoom>{
    ...
    createRoom(sceneId: number | string, roomId: string) {
        const room = {
            roomId,
            sceneId,
            nid: GameNidEnum.Erba,
            serverId: pinus.app.getServerId(),
        }
        const sceneInfo: IsceneMgr = getConfiguration('scenes/Erba').datas.find(scene => scene.id === sceneId);
     ...
     * @param player
     */
    check(room: ErbaRoom, player): boolean {
...
export default new ErRoomManger(GameNidEnum.Erba, InteriorGameType.Battle, 'Erba');
