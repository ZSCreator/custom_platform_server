...
export class cpRoomManger extends TenantRoomManager<cpRoom>{
    ...
    createRoom(sceneId: number | string, roomId: string) {
        const room = {
            roomId,
            sceneId,
            nid: GameNidEnum.ChinesePoker,
            serverId: pinus.app.getServerId(),
        }
        const sceneInfo: IsceneMgr = getConfiguration('scenes/chinese_poker').datas.find(scene => scene.id === sceneId);
     ...
     * @param player
     */
    check(room: cpRoom, player): boolean {
...
export default new cpRoomManger(GameNidEnum.ChinesePoker, InteriorGameType.Battle, 'ChinesePoker');