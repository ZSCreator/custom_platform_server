...
import {Lottery, LWLotteryResult} from "./util/lotteryUtil";
...
export default class Control extends SlotGameControl<LimitConfigManager> {
    ...
    async runControl(player: Player, lotteryUtil: Lottery): Promise<LWLotteryResult> {
        let result: LWLotteryResult, controlType: ControlKinds;
        ...
        const { isOverrun }: { isOverrun: boolean } =
            this.isEarningsTransfinite({ player, profit: result.profit });
