...
import SambaGameManager from "./lib/sambaGameManager";
...
class Lifecycle implements ILifeCycle {
    async beforeStartup(app: Application, cb: () => void) {
         ...
         * 初始化游戏以及房间和场
         */
        await new SambaGameManager(GameNidEnum.Samba).init();
        // }
        cb();
    }
    ...
    async afterStartup(app: Application, cb: () => void): Promise<any> {
        ...
        // 初始化调控
        await GameControlService.getInstance().init({nid: GameNidEnum.Samba});
