...
export class MainHandler {
    ...
    /**
     * 申请奖池信息
     * @route: Samba.mainHandler.jackpotFund
     */
     ...
     * @param room
     * @param player
     * @route: Samba.mainHandler.getSambaAward
     */
    async getSambaAward({ room, player }: { room: RoomStandAlone, player: Player, }) {
        console.warn('获取桑巴奖励', player.uid, player.gameState);
        // 如果不是博一博状态
        if (player.gameState !== PlayerGameState.FREE) {
            return { code: 500, error: getlanguage(player.language, Net_Message.id_3103) };
        }
