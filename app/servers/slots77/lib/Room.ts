...
import { crateSlotLottery, SlotResult, personalInternalControl ,goldDoubleResult } from './util/lotteryUtil';
...
export default class RoomStandAlone extends BaseSlotMachineRoom<Player>{
    ...
    async lottery(_player: Player): Promise<SlotResult> {
        ...
        // 获取调控结果
        let result = await ControlImpl.getControlInstance(this).runControl(_player, lotteryUtil);
        if(result.totalWin > 0){
            let doubleResult = goldDoubleResult(result.totalWin);
            if(doubleResult){
     ...
     * 金币加倍
     */
    async goldDouble(_player: Player){
         //将结果返回
        ...
        if(result.finallyWin > 0){
            ...
            let doubleResult = goldDoubleResult(_player.playerWin);
            if(doubleResult){
