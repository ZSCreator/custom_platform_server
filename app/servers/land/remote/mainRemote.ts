...
export class mainRemote {
    ...
    async exit({ nid, sceneId, roomId, player }) {
        ...
        if (playerInfo) {
            if (roomInfo.status == "INGAME" || roomInfo.status == "CPoints" ||
                roomInfo.status == "DOUBLE") {
                return { code: 500, msg: langsrv.getlanguage(playerInfo.language, langsrv.Net_Message.id_1052) };
            }
            // console.warn(`uid:${currPlayer.uid}离开房间`);
            roomInfo.leave(playerInfo, false);
        }
        // roomInfo && RoomController.updateUserFromRoom(roomInfo.players, roomInfo.nid, roomInfo.roomId);
        return { code: 200, msg: '' };
    };
    ...
    */
    public async rpcLowerPlayer({ uid, sceneId, roomId }) {
        try {
            ...
            // Logger.error('land|async  rpcLowerPlayer==>', error);
            return { code: 200 };
        }
    };
    /**掉线回来 记录 不踢人 */
