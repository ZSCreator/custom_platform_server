...
import {Lottery, ScratchCardResult} from './util/lotteryUtil'
...
export default class ControlImpl extends BaseGameControl {
    ...
    public static getControlInstance(): ControlImpl {
        if (this.instance === null) {
            this.instance = new ControlImpl({room: { nid: GameNidEnum.Scratch, sceneId: 0 }} );
        }
    ...
    async runControl(player: Player, lotteryUtil: Lottery): Promise<ScratchCardResult> {
        let result: ScratchCardResult, controlType: ControlKinds;
