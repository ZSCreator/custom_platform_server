...
/**
 * 房间场控状态
 * @property NONE 不调控 随机发牌
 * @property SCENE 场控
 * @property PERSONAL 个控
 */
...
/**
 * 21点调控实现
 * @property roomControlState 当前房间调控状态
...
export default class Control extends BaseGameControl {
    ...
    constructor({room}: { room: BlackJackRoomImpl }) {
        super({room});
    }
    ...
    protected stripPlayers(): ControlPlayer[] {
        return this.room.players.filter(p => (p.isRobot === RoleEnum.REAL_PLAYER && p.totalBet > 0)).map(p => filterProperty(p));
    }
}