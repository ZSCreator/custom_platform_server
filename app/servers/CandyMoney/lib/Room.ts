...
export default class RoomStandAlone extends BaseSlotMachineRoom<Player> {
    ...
    async settlement(_player: Player, result: PharaohLotteryResult) {
        ...
        const record = buildRecordResult("SPIN", result.winningDetails, result.odds);
        ...
        for (let i = 0, len = result.freeSpinResult.length; i < len; i++) {
            ...
            if (freeSpinWin > 0) {
                const record = buildRecordResult("FREE", result.freeSpinResult[i].winningDetails, result.freeSpinResult[i].odds);
