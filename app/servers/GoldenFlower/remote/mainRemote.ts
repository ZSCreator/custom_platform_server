...
export class mainRemote {
    ...
    */
     ...
     * 离线 - 直接强行离开
     */
    public async leave({ uid, group_id, lineCode, sceneId, roomId }) {
        const roomInfo = this.roomManager.searchRoom(sceneId, roomId);
        if (!roomInfo) {
            return { code: 200, msg: `not find roomCode:${roomId}` };
        }
        ...
        if (!playerInfo) {
            this.roomManager.removePlayer({uid, lineCode, group_id} as PlayerInfo);
    ...
    */
    public async rpcLowerPlayer({ uid, sceneId, roomId }) {
        try {
            ...
            log_logger.error('JH|async  rpcLowerPlayer==>', error);
            return { code: 200 };
        }
    };
    /**掉线回来 记录 不踢人 */
