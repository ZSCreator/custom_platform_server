...
import { RoomState } from "./qzpjConst";
...
export class qzpjRoomManger extends TenantRoomManager<qzpjRoom>{
    ...
    createRoom(sceneId: number | string, roomId: string) {
        const room = {
            roomId,
            sceneId,
            nid: GameNidEnum.qzpj,
            serverId: pinus.app.getServerId(),
        }
        const sceneInfo: IsceneMgr = getConfiguration('scenes/qzpj').datas.find(scene => scene.id === sceneId);
     ...
     * @param player
     */
    check(room: qzpjRoom, player): boolean {
        const result = super.check(room, player);

        // 如果前面检查都通过
        if (result && (room["status"] !== RoomState.INWAIT && room["status"] !== RoomState.NONE)) {
            return false;
        }
...
export default new qzpjRoomManger(GameNidEnum.qzpj, InteriorGameType.Battle, 'qzpj');